...
```{r}
(ac_imf = ac %>% filter(year>1965, independence=TRUE))
```
...
```{r}
ac_imf = inner_join(ac_imf, export_df_joinable, by = c("country", "cc3", "year"))
```
...
ac_imf = inner_join(ac_imf, import_df_joinable, by = c("country", "cc3", "year"))
...
### Transform Index to Percentage Change
TODO: declan will write dplyr code to transform our inflation_annual_* columns to pct_change_* representing the percent change in the index from the previous year to the current (a double between 0. and 1.0)
