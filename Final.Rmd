...
1. `case` is a integer variable that identifies the index for each country from the original, global dataset.  
...
5. `systemic_crisis` is a logical indicator variable that indicates the occurrence of a systemic crisis: significant signs of financial distress in a majority of the banking system coupled with signficant banking policy intervention measures implemented by financial authorities in response. 

6. `exch_usd` is a double variable expressing the exchange rate of USD to the associated country's currency. 

7. `domestic_debt_in_default` is a logical indicator variable that indicates the occurrence of a failure of government in repyament of its domestic debts owed to domestic residents of the country. 

8. `sovereign_external_debt_default` is a logical indicator variable that indicates the occurrence of a failure of government to honor some or all of its debt obligations to the external lenders.

9. `gdp_weighted_default` is a double variable in the data but its meaning is unclear. 

10. `inflation_annual_cpi` is a double variable that expresses the percent change in the consumer price index, generally used as a measure of inflation.

11. `independence` <logical>

12. `currency_crises` is a logical indicator variable that indicates the occurrence of serious doubt as to whether a country's central bank has sufficient foreign exchange reserves to maintain the country's fixed exchange rate.

13. `inflation_crises` is a logical indicator variable that indicates the occurrence of an inflation crisis which means very high and typically accelerating inflation.

14. `banking_crisis` is a logical indicator variable that indicates whether a banking crisis (demise of a financial institution) occurred. 
...
We create new data sets for this exploratory data analysis so as not to affect previous data sets. 
...
We use data summaries, histograms, box and whisker plots, and scatter plots (outputs hidden) to ensure that there are no errors in the data and no outlying observations that we wish to remove from the data to be analyzed.
...
## How does Systemic Crisis depend on time and independence?
...
Let's build a custom simple logistic regression so we can better understand what the built-in R functions are doing and better interpret their results.
...
paste("Specificity", mean(preds[Y==0] == Y[Y==0]))
...
We end up with the same results from the custom and the built-in logistic regression functions, as desired.
...
Here are the results and a helpful visualization of the bootstrapped distribution of b1, the regression coefficient for banking crisis. 
...
Clearly, zero falls outside our bootstrapped confidence interval, indicating that B1 is significantly different than zero, and that banking crisis is a significant predictor of systemic crisis in our data.


## Multiple Logistic Regression (MLR) Model

Now let's build the multiple logistic regression model using all our predictor variables. 
...
```{r}
# Let's optimize on a balanced sensitivity (true positives) and specificity (true negatives)
...
What happens if we wish to predict systemic crisis without the aid of data on banking crisis, as might be the case in a real financial crisis situation?
...
Our model had between .75 and .8 training accuracy, sensitivity, and specificity. Though we were able to predict 63 out of 82 crises, we also had 208 false positives, and that magnitude is troubling. We have to think hard about how to balance the errors based on the costs of incorrect prediction, whether false positives or false negatives. 

Let's look at the effect that each predictor has on the probability of a systemic crisis, given the values of the other predictor variables are held fixed:
...
Banking crisis was the most significant predictor of systemic crises. Dependency on export revenue made countries more likely to experience economic crises, in accordance with some economic theory.

However, our model made several assumptions. We distilled economic variables to logical values, which means we lose nuanced information. We did not take into account the time series, which only had year to year data and would not help with prediction on a a different time scale such as week or month. 

For future analysis, we would like to investigate further economic variables with more granularity and perhaps model complexity such as interaction terms and diagnostic/remedial measures to help with model selection. We might also want to do further modeling and prediction with new samples from other countries. 